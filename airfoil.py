...
class BaseFitter:
    ...
    def __init__(self, csv_filename):
        # Read x, y coordinates from a standard 2-column CSV
        data = np.loadtxt(csv_filename, delimiter=',')

        # True coordinates of the airfoil
        self.xcoords_orig = data[:, 0]
        self.ycoords_orig = data[:, 1]
    ...
    def get_points(self, scale_factor=1, translation=(0, 0), rotation_angle=0):
        # If the points have not been fitted by a suitable model, the same data points are used.
        ...
        # # Splitting the upper and lower points with respect to the 
        # le_idx = np.argmin(self.x_norm)
        # xu = x_final[:le_idx + 1]
        # yu = y_final[:le_idx + 1]
        # xl = x_final[le_idx:]
        # yl = y_final[le_idx:]
...
class CST(BaseFitter):
    ...
    def fit(self, n_tries=5, resolution=None):
        ...
        # If a resolution is provided, a cosine spacing is used.
        if resolution:
...
def get_naca_symmetric(thickness=12, resolution=200, scale_factor=1.0, translation=(0, 0), rotation_angle=0):
    # Cosine spacing
    x = 0.5 * (1.0 - np.cos(np.linspace(0, np.pi, resolution)))
    t = thickness / 100.0
    ...
    # # 6. Split back into upper and lower bounds
    # le_idx = len(xu_norm) - 1
